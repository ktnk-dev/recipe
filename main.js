...
async function init() {
    ...
    await fetch('./collection.json').then(e => {return e.json()}).then(async d => {
        collections.data = d
        var total_recipes = 0
        Object.keys(d).forEach(c => {total_recipes += Object.keys(d[c].recipes).length})
        try {document.querySelector('#inbase').innerHTML = `В базе ${total_recipes} рецептов и ${Object.keys(d).length} коллекций`} catch {}
...
const main = {
    clear: async (direction) => {
        if (debug) console.log('Clearing main element innerHTML')
        document.querySelectorAll('main > *').forEach(e => {
            e.style.animation = `hide${direction} .25s cubic-bezier(.99,-0.02,.54,.74)`
        })
        await sleep(240)
    ...
    render: async (innerHTML, direction) => {
        if (debug) console.log('Rendering new main element innerHTML')
        if (direction == animations.to_left_side) {direction = animations.to_right_side}
        else {direction = animations.to_left_side}
        document.querySelector('main').innerHTML = innerHTML
        document.querySelectorAll('main > *').forEach(e => {
            e.style.animation = `hide${direction} .25s cubic-bezier(.99,-0.02,.54,.74) reverse`
        })
        await sleep(240)
        document.querySelectorAll('main > *').forEach(e => {
            e.style.animation = ``
...
<div class="recipe_mini" onclick="app.showRecipe('${id}')">
    <h2>${recipe.name}</h2>
    <div class="info">
        <span>${recipe.data.calories} ккал.</span>
        <span>${recipe.collection}</span>
        <span>${recipe.data.type}</span>
    </div>
</div>
        `
    },
...
const app = {
    ...
    default: async () => {
        ...
        const total_collections = Object.keys(collections.data).length
        return `
        <h1>Рецепты</h1>
        <h3>Сайт с огромным количеством рецептов, отсортированные по коллекциям</h3>
        <h3 id="inbase">В базе ${total_recipes} рецептов и ${total_collections} коллекций</h3>
        <h3>Выбери нужный пункт в самом низу экрана, чтобы продолжить</h3>`
    },
    back: async () => {
        if (!current_page_handler) {current_page_handler = app.default}
